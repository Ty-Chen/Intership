...
bool KSearch::SearchBm(char* pszFilePath)
{
    ...
    else
    {
        ...
        while (true)
        {
            ...
            {
                bRetCode = pBmTest->Search(m_pszPattern, m_nPatternLen,
                    m_pszFirstReadBuffer + nLastBufferPos, nFirstSize - nLastBufferPos);
                KGLOG_PROCESS_ERROR(bRetCode);
                break;
            }
            ...
            bRetCode = pBmTest->Search(m_pszPattern, m_nPatternLen, m_pszFirstReadBuffer + nLastBufferPos,
                nBufferPos + 1 - nLastBufferPos);
            KGLOG_PROCESS_ERROR(bRetCode);
            nLastBufferPos = (nBufferPos - nFirstSize) == 0 ? 0 : (nBufferPos - nFirstSize + 1);
            std::swap(m_pszFirstReadBuffer, m_pszSecondReadBuffer);
            std::swap(nFirstSize, nSecondSize);
            /*if (bFirstBuffer)
            {
                if (nFirstSize < MAX_BUFFER_SIZE)
                {
...
bool KSearch::SearchBmTwo(char* pszFilepath)
{
    bool      bResult        = false;
    bool      bRetCode       = false;
    int       nFirstSize     = 0;
    ...
    nFirstSize = fread(m_pszFirstReadBuffer, sizeof(char), MAX_BUFFER_SIZE, fpFile);
    m_pszFirstReadBuffer[nFirstSize] = '\0';
    KGLOG_PROCESS_ERROR(nFirstSize >= 0);
    ...
    if (nFirstSize < MAX_BUFFER_SIZE)
    {
        bRetCode = pBmTest->SearchSunday(m_pszPattern, m_nPatternLen, m_pszFirstReadBuffer, nFirstSize);
        KGLOG_PROCESS_ERROR(bRetCode);
    }
    else {
        m_pszSecondReadBuffer = new char[MAX_PATTERN + 1];
        KGLOG_PROCESS_ERROR(m_pszSecondReadBuffer);

        while (true)
        {
            if (m_pszFirstReadBuffer[nFirstSize - 1] != '\n' && nFirstSize == MAX_BUFFER_SIZE)
            {
                fgets(m_pszSecondReadBuffer, MAX_PATTERN + 1, fpFile);
                nSecondSize = strlen(m_pszSecondReadBuffer);

                for (int i = 0; i < nSecondSize; i++)
                {
                    m_pszFirstReadBuffer[nFirstSize++] = m_pszSecondReadBuffer[i];
                }
            }
            m_pszSecondReadBuffer[nFirstSize] = '\0';
            bRetCode = pBmTest->SearchSunday(m_pszPattern, m_nPatternLen,
                m_pszFirstReadBuffer, nFirstSize);
            KGLOG_PROCESS_ERROR(bRetCode);
