...
bool KBm::BoyerMoore(char* pszPattern, int nPatternLen, char* pszText, int nTextLen)
{
    ...
    for (int i = 0; i <= nTextLen - nPatternLen;)
    {
        ...
        if (nMatchPos < 0)
        {
            for (nTextPos = i; nTextPos >= 0; nTextPos--)
            {
                if (pszText[nTextPos] == '\n')
                {
                    break;
            ...
            nTextPos = nTextPos + 1;
            for (int j = nTextPos; j <= nTextLen; j++)
            {
                ...
                pszResult[nMatchTextPos] = pszText[j];
                nMatchTextPos++;
            }
    ...
    nRetCode = fwrite(pszResult, sizeof(char), nMatchTextPos, fpFile);
    KGLOG_PROCESS_ERROR(nRetCode == nMatchTextPos);
